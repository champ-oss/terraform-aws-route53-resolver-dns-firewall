  ...
  container_definitions = jsonencode([
    {
      ...
      command = [
        "sh",
        "-c",
        "dnf install -y bind-utils >/dev/null 2>&1 && dig amazonaws.com >/dev/null && dig github.com >/dev/null && dig api.github.com >/dev/null && dig www.example.com >/dev/null && tail -f /dev/null"
      ]
    }
  ])
